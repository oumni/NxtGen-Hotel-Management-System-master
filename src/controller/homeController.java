...
public class homeController extends CommonMethods implements Initializable{
    ...
    @FXML
    private JFXButton  closebtn, minimisebtn, moviescreensbtn,s1issuebtn,s1issuebtn1, s2issuebtn, s3issuebtn,addbtn,registerbtn;
    ...
    @FXML
    private ImageView s1movieimage,s1movieimage1, s2movieimage, s3movieimage;
    @FXML
    ...
    public void setIssueTicketsActions(){
        ...
        s1issuebtn1.setCursor(Cursor.HAND);
        
        s1issuebtn1.setOnAction(event1 -> {
            SequentialTransition st=popButton(s1issuebtn1, 1.2);
            st.play();
            st.setOnFinished(event -> {
                checkoutController.selectedmovie=s1moviename.getText();
                checkoutController.selectedtime="";
    ...
    public void populateScreens(){
         ...
         s1movieimage1.setImage(server.getImage(server.getCurrentMovieName("SCREEN1")));

        //s s1moviename.setText(server.getTitle(server.getCurrentMovieName("SCREEN1")));
    ...
    private void popNode(AnchorPane pane, AnchorPane shadow){
        ...
        pane.setOnMouseEntered(event -> {
            ...
            timeline.getKeyFrames().add(new KeyFrame(Duration.millis(500),
                    new KeyValue (shadow.minWidthProperty(), 200)));
            timeline.play();
